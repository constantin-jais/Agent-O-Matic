...
deploy), each behind its own envelope. A7 chains them into a single `aom loop`
under one global envelope.

## Decision

`aom loop` runs dispatch -> automerge -> deploy and **stops at the first stage
...
- **Global envelope** — on top of each stage's own: a loop kill-switch
  (`AOM_LOOP_DISABLED`), a scope-fence, and a global circuit-breaker (max
  iterations).
...
- Each link stays usable standalone (`aom incident|dispatch|automerge|deploy`);
