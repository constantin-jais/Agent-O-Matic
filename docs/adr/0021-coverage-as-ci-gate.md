...
- The floor is intentionally conservative. The orchestrator's live-boundary
  wrappers (the gh/git/claude subprocess seams: the `GhChecksGate` poll loop,
  `GhMerger`, `GhPublisher`, `ClaudeFixer`, `GithubForge`, `CommandDeployer`,
  `RealStages`) are deliberately not unit-covered — they are the I/O boundary,
  proven live, not in tests. Overall line coverage is bounded below 100 by design;
  the floor sits under the real figure and ratchets up as coverage grows.
